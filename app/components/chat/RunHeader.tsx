...
const AgentChip = ({ agent }: { agent: any }) => {
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'working': return 'bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.6)] animate-pulse';
...
export const RunHeader: React.FC<RunHeaderProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="h-1 bg-[#222] w-full mt-14 absolute top-0"> {/* Should be relative? No, fixed structure */}
      </div>
        ...
        <div className="flex items-center justify-between">
           <div className="flex items-center gap-3">
              <div className="w-10 h-10 flex items-center justify-center rounded-lg bg-[#222] border border-[#333]">
                {progress >= 100 ? <CheckCircle2 className="w-5 h-5 text-green-400" /> : <RotateCw className="w-5 h-5 text-blue-400 animate-spin-slow" />}
              </div>
              <div>
           ...
           <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-4 text-xs">
                ...
                <div className="flex items-center gap-1.5 text-gray-400" title="QC Issues">
                   <AlertTriangle className={`w-3.5 h-3.5 ${stats.qcIssues.critical > 0 ? 'text-red-500' : 'text-gray-400'}`} />
                   <div className="flex gap-1">
                      {stats.qcIssues.critical > 0 && <span className="text-red-400 font-bold">{stats.qcIssues.critical}C</span>}
