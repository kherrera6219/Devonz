            ...
            <div className="p-4 space-y-4">
                {stages.map((stage, i) => (
                    <div key={i} className={`flex gap-3 ${stage.status === 'pending' ? 'opacity-40' : 'opacity-100'}`}>
                         <div className="mt-1">
                            {stage.status === 'completed' && <div className="w-5 h-5 rounded-full bg-green-900/30 border border-green-500/50 flex items-center justify-center"><Check className="w-3 h-3 text-green-400"/></div>}
                            {stage.status === 'running' && <div className="w-5 h-5 rounded-full border-2 border-blue-500 border-t-transparent animate-spin"/>}
                            {stage.status === 'pending' && <div className="w-5 h-5 rounded-full border border-gray-700"/>}
                            {stage.status === 'failed' && <div className="w-5 h-5 rounded-full bg-red-900/30 border border-red-500/50 flex items-center justify-center"><X className="w-3 h-3 text-red-400"/></div>}
                         </div>
                         <div>
                ...
                <div className="bg-[#111] p-3 border-t border-[#222] flex items-center justify-between">
                    {lastQC ? (
                        <div className="flex items-center gap-4 text-xs">
                             ...
                             <div className="flex gap-3">
                                <span className={`${lastQC.critical > 0 ? 'text-red-400 font-bold' : 'text-gray-400'}`}>{lastQC.critical} Critical</span>
                    ...
                    {onAction && (
                       <div className="flex gap-2">
                           <button onClick={() => onAction('view_changes')} className="flex items-center gap-1.5 px-3 py-1.5 bg-[#222] hover:bg-[#333] border border-[#333] rounded text-xs text-gray-300 transition-colors">
