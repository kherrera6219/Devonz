...
class LogStore {
  ...
  logTaskOperation(taskId: string, operation: string, status: string, details?: Record<string, unknown>) {
    return this._addLog(
      ...
      'info',
      'task',
      { taskId, operation, status, ...(details as Record<string, unknown>) },
      {
  ...
  logProviderAction(provider: string, action: string, success: boolean, details?: Record<string, unknown>) {
    return this._addLog(
      ...
      success ? 'info' : 'error',
      'provider',
      { provider, action, success, ...(details as Record<string, unknown>) },
      {
        component: 'providers',
  ...
  logPerformanceMetric(component: string, operation: string, duration: number, details?: Record<string, unknown>) {
    return this._addLog(
      ...
      duration > 1000 ? 'warning' : 'info',
      'performance',
      { component, operation, duration, ...(details as Record<string, unknown>) },
      {
        component,
